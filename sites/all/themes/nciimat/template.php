 ...
 * Implements hook_form_alter().
 */
function nciimat_form_alter(array &$form, array &$form_state = array(), $form_id = NULL) {
  if ($form_id) {
    switch ($form_id) {
      case 'search_form':
 ...
 * @see hook_theme()
 */
function nciimat_theme(&$existing, $type, $theme, $path) {
  // Custom theme hooks:
...
function nciimat_preprocess_page(&$variables) {
...
function nciimat_links__system_main_menu($variables) {
  $html = "<div>\n";
  $html .= "  <ul>\n";  
...
/**
    ...
    */
function nciimat_preprocess_search_result(&$variables) {
